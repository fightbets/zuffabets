      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-14">
          ...
          <div className="hidden md:flex items-center gap-1">
            <a
              href="https://fightbets.com?source=zuffabets-v3"
              target="_blank"
              rel="noopener noreferrer"
              className="bg-gold hover:bg-gold-dark text-white px-4 py-1.5 rounded text-xs font-bold transition no-underline"
            >
              FightBets
            </a>
          </div>
          ...
          {navItems.map(item => {
            ...
            return (
              <Link
                key={item.path}
                to={item.path}
                className={`v3-nav-item no-underline ${
                  isActive ? 'v3-nav-active text-gold-dark' : 'text-gray-600'
                }`}
              >
                {item.label}
              </Link>
          ...
          <div className="px-4 py-2">
            {navItems.map(item => (
              <Link
                ...
                className={`block px-3 py-3 text-sm font-medium border-b border-gray-50 no-underline transition ${
